...
- **当前版本**：v0.1 (AI 初版)
- **翻译技术**：基于 Gemini 3 Flash 模型进行全文本自动化处理。
- **项目状态**：已完成全文本 AI 翻译，正处于人工校对阶段。
...
├── GLOSSARY.md             # 汉化术语表（确保 Aurifer、Sanctifier 等专有名词译名统一）
└── README.md               # 项目说明文档
...
2. 解包资源 (Unpack)
将正版游戏的 app.asar 文件放入项目根目录。

运行 unpack.bat，脚本会将资源解压至 unpacked/ 文件夹。
...
脚本将使用专业命令排除 .node、.dll 等二进制文件，生成新的 app.asar 并备份原件。
...
定位目录：在 Steam 库中右键点击游戏 -> 管理 -> 浏览本地文件。进入 resources 文件夹。

备份原件：将原始 app.asar 重命名为 app.asar.bak。

覆盖补丁：将下载的汉化版 app.asar 放入该目录。

运行要求：确保 app.asar 与解包时生成的 app.asar.unpacked 文件夹处于同一级目录下，否则游戏将无法启动。

🤝 协作规范
译名统一：修改文本前请查阅 GLOSSARY.md。

提交修改：建议通过 Pull Request 提交润色后的 .txt 文件。

Bug 反馈：若遇到逻辑死循环或变量报错（如 expected 2 was 4），请提交 Issue 并附带报错截图。

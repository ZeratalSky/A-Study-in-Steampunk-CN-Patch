...
function showStats() {
    ...
    if (button && button.innerHTML == "返回游戏") {
...
function showMenu() {
  ...
  function menu() {
    ...
    var button = document.getElementById("menuButton");
    button.innerHTML = "返回游戏";
    options = [{name:"返回游戏", group:"choice", resume:true}];
    if (window.isSteamworks) {
      options.push(
        {name:"退出游戏", group:"choice", quit:true}
      )
    }
    if (nav.achievementList.length) options.push(
      { name: "查看成就列表", group: "choice", achievements: true }
    );
    options.push(
      {name:"重新开始游戏", group:"choice", restart:true},
      {name:"游戏设置", group:"choice", settings:true},
      {name:"玩更多类似游戏", group:"choice", moreGames:true}
    );
    if (isWebSavePossible()) {
      if (!window.isSteamApp) {
        options.push(
          { name: "联系我们" + getSupportEmail() + ".", group: "choice", contactUs: true },
        )
      }
      options.push(
        { name: "错误反馈", group: "choice", reportBug: true },
        { name: "分享这款游戏", group: "choice", share: true },
      )
    }
    options.push(
      {name:"有新游戏时通知我", group:"choice", subscribe:true},
      {name:"显示键盘快捷键", group:"choice", shortcuts:true}
    );
    if (document.getElementById("aboutLink")) {
      options.push({name:"查看制作人员名单", group:"choice", credits:true});
    }
    var logoutLink = document.getElementById("logout");
    if (logoutLink && logoutLink.style.display !== "none") {
      options.push({ name: "退出登录", group: "choice", logout: true });
    }
    printOptions([""], options, function(option) {
      if (option.resume) {
        ...
        if (_global.blockRestart) {
          asyncAlert("请等待计时结束后再尝试。");
          return;
        }
        return clearScreen(function() {
          var text = document.getElementById('text');
          text.innerHTML = "确定要从头开始游戏吗？";
          var options = [
            {name: "确定重新开始", group:"choice", restart: true},
            {name: "取消", group: "choice", restart: false },
          ]
          printOptions([""], options, function(option) {
            if (option.restart) {
              clearScreen(function() {
    ...
    if (window.isSteamApp) {
      printParagraph("需要帮助？请发送邮件至" + getSupportEmail() + ".");
    }
    curl();
  }
  clearScreen(menu);
...
function textOptionsMenu(categories) {
  ...
  clearScreen(function() {
    ...
    if (categories.settings) {
      text.innerHTML = "<p>修改游戏设置。</p>";
    } else if (categories.size && categories.color) {
      text.innerHTML = "<p>修改游戏外观。</p>";
    } else if (categories.size) {
      text.innerHTML = "<p>增大或缩小字体。</p>";
    } else if (categories.color) {
      text.innerHTML = "<p>更换背景颜色。</p>";
    } else if (categories.animation) {
      text.innerHTML = "<p>更改翻页动画。</p>";
    } else if (categories.sliding) {
      text.innerHTML = "<p>启用或禁用触摸滑动控制。</p>";
    }
    options = [
      {name:"返回游戏", group:"choice", resume:true},
    ];
    if (categories.size) {
      options.push(
        {name:"增大字体", group:"choice", bigger:true},
        {name:"缩小字体", group:"choice", smaller:true}
      );
      if (oldZoom <= 0.5) {
        options[options.length-1].unselectable = true;
      }
      if (oldZoom !== 1) {
        options.push({name:"恢复默认字号", group:"choice", reset:true});
      }
    }
    if (categories.family) {
      if (oldFontFamily !== "serif") options.push({ name: "使用有衬线字体 (Serif)", group: "choice", family: "serif" });
      if (oldFontFamily !== "sans") options.push({ name: "使用无衬线字体 (Sans-serif)", group: "choice", family: "sans" });
      if (oldFontFamily !== "dyslexia") options.push({ name: "使用易读性字体 (Dyslexia-friendly)", group: "choice", family: "dyslexia" });
    }
    if (categories.color) options.push(
      {name:"使用黑色背景", group:"choice", color:"black"},
      {name:"使用羊皮纸色背景", group:"choice", color:"sepia"},
      {name:"使用白色背景", group:"choice", color:"white"}
    );
    if (categories.animation) {
      if (window.animateEnabled) {
        options.push({ name: "关闭翻页动画", group: "choice", animation: 2 });
      } else {
        options.push({ name: "开启翻页动画", group: "choice", animation: 1 });
      }
    }
    if (categories.sliding) {
      if (window.slidingEnabled !== false) {
        options.push({ name: "禁用触摸滑动", group: "choice", sliding: 1 });
      } else {
        options.push({ name: "启用触摸滑动", group: "choice", sliding: 2 });
      }
    }
    printOptions([""], options, function(option) {
      if (option.resume) {
...
function printFooter() {
  ...
  if (statsButton) {
    if (window.stats && stats.scene && stats.scene.secondaryMode == "stats") {
      statsButton.innerHTML = "返回游戏";
    } else {
      statsButton.innerHTML = "查看属性";
...
function achieve(name, title, description) {
  // 1. 【核心修复】强制去 achievements 数组里重新查一遍数据
  // 这样无论传进来的是什么垃圾数据，我们都用 mygame.js 里的真值覆盖它
  if (window.achievements) {
    for (var i = 0; i < window.achievements.length; i++) {
      // 数组第0位是ID，如果匹配到了...
      if (window.achievements[i][0] == name) {
        title = window.achievements[i][3];       // 第3位是中文标题
        description = window.achievements[i][4]; // 第4位是中文描述
        break;
      }
    }
  }
  ...
  // 3. 【汉化美化】将原本的 "Achievement:" 改为 "获得成就："
  var html = "<b>获得成就： "+escapedTitle+"</b><br>" + escapedDescription;
  alertify.log(html);
}
...
function changeAuthor(author) {
  ...
  if (authorTag) {
    authorTag.innerHTML = "";
    authorTag.appendChild(document.createTextNode("作者：" + author));
  }
  var text = document.getElementById('text');
  if (text) {
    var textAuthorTag = document.createElement('h2');
    textAuthorTag.classList.add('gameTitle');
    textAuthorTag.appendChild(document.createTextNode("作者：" + author));

...
Scene.prototype.restore_game = function restore_game(data) {
  ...
  function renderRestoreMenu(saveList, dirtySaveList) {
    ...
    if (false) options.push({name:"使用密码恢复存档。", group:"choice", password:true});
    options.push({name:"从 choiceofgames.com 获取云端存档。", group:"choice", fetch:true});
    if (anythingToDelete) options.push({ name: "删除存档。", group: "choice", deleting: true });
    if (recentlyDeletedCount) {
      var recentlyDeletedMessage;
      if (recentlyDeletedCount === 1) {
        recentlyDeletedMessage = "1 个存档近期已删除。"
      } else {
        recentlyDeletedMessage = recentlyDeletedCount + " 个存档近期已删除。"
      }
      options.push({ name: "撤销删除。 (" + recentlyDeletedMessage + ")", group: "choice", undeleting: true });
    }
    if (dirtySaveList.length) options.push({name:"上传存档至 choiceofgames.com。", group:"choice", upload:true});
    options.push({name:"取消。", group:"choice", cancel:true});
    var groups = [""];
      ...
      function synchronizeBeforeEdit(renderMenu) {
        clearScreen(function () {
          fetchEmail(function (defaultEmail) {
            self.printLine("请输入您的电子邮件地址以验证身份。");
            self.paragraph();
